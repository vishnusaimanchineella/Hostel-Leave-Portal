...
def users_list(request):
	...
	return render(request,'accounts/users_table.html',{'students':students,'title':'Users List'})


def users_unblock(request,id):
	user = get_object_or_404(User,id = id)
	std = Student.objects.filter(user = user).first()
	std.is_blocked = False
	std.save()
	user.is_active = True
	user.save()
...
def users_blocked_list(request):
	...
	return render(request,'accounts/all_deleted_users.html',{'students':blocked_students,'title':'blocked users list'})