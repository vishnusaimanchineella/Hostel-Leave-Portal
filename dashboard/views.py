...
def dashboard_students(request):
	...
	paginator = Paginator(students, 10) #show 10 employee lists per page
	...
	blocked_students = Student.objects.all_blocked_students()


	return render(request,'dashboard/student_app.html',dataset)




def dashboard_students_create(request):
	if not (request.user.is_authenticated and request.user.is_superuser and request.user.is_staff):
		return redirect('/')
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			instance.studentid = request.POST.get('studentid')
			...
			messages.error(request,'Trying to create dublicate students with a single user account ',extra_tags = 'alert alert-warning alert-dismissible show')
			return redirect('dashboard:studentcreate')


	dataset = dict()
	form = StudentCreateForm()
	dataset['form'] = form
	dataset['title'] = 'register student'
	return render(request,'dashboard/student_create.html',dataset)
...
def student_edit_data(request,id):
	...
	if request.method == 'POST':
		form = StudentCreateForm(request.POST or None,request.FILES or None,instance = student)
		if form.is_valid():
			instance = form.save(commit = False)
	...
	form = StudentCreateForm(request.POST or None,request.FILES or None,instance = student)
	dataset['form'] = form
	dataset['title'] = 'edit - {0}'.format(student.get_full_name)
	return render(request,'dashboard/student_create.html',dataset)
...
def dashboard_student_info(request,id):
	if not request.user.is_authenticated:
		return redirect('/')
	...
	dataset['title'] = 'profile - {0}'.format(student.get_full_name)
	return render(request,'dashboard/student_detail.html',dataset)
...
def leaves_view(request,id):
	...
	print(leave.user)
	student = Student.objects.filter(user = leave.user)[0]
	print(student)
	return render(request,'dashboard/leave_detail_view.html',{'leave':leave,'student':student,'title':'{0}-{1} leave'.format(leave.user.username,leave.status)})
...
def approve_leave(request,id):
	...
	messages.error(request,'Leave successfully approved for {0}'.format(student.get_full_name),extra_tags = 'alert alert-success alert-dismissible show')
